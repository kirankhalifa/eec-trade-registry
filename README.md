...
The active implementation includes the unauthenticated public catalogue, Discord OAuth staff sign-in with database-authorized staff operations, exact-reference public dealer and license verification, credential-based dealer access with effective-dated representation, an audited staff licensing office, wholesale order intake, warehouse ledger and reservation operations, fungible fulfillment, and one-way projection integrations. Dealers can submit and track requisitions without price or stock on hand; authorized staff can review them, post balanced fungible receipts, derive on-hand/available positions, manage 48-hour stock reservations, and atomically consume a reservation into a balanced stock issue and completed order state. A leased worker replaces approved public Google Sheet tabs and delivers allowlisted Discord alerts, while signed Discord commands expose only the existing public catalogue and verification contracts. Applications, renewal, serialized-asset receipt and custody, transfers, reconciliation, compliance cases, and generated documents remain future policy-gated work.
