...
export default async function DashboardPage() {
  ...
  return <main className="staff-main">
    ...
    <div className="dashboard-layout"><div>
      <section className="dashboard-panel"><header className="dashboard-panel-header"><div><h2>Recent orders</h2><p>Open an order once and keep working there until handoff.</p></div><Link href="/staff/orders">View all</Link></header>{dashboard.recent_orders.length > 0 ? <ul className="dashboard-activity-list">{dashboard.recent_orders.map((order)=><li key={order.id}><Link className="dashboard-activity-item" href={`/staff/orders/${order.id}`}><span className="dashboard-activity-mark"/><span><strong>{order.reference} · {order.customer}</strong><small>{order.status.replaceAll("_"," ")} · {order.channel.replaceAll("_"," ")} · <RelativeTime value={order.submitted_at}/></small></span></Link></li>)}</ul> : <div className="dashboard-empty dashboard-empty-action"><p>No orders have been submitted yet.</p><Link className="button button-primary button-compact" href="/staff/orders/new">Create the first order</Link></div>}</section>
    </div><aside>
