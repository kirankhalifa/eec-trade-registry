...
function destination(key: "error" | "notice", value: string) {
  return `${assetPath}?${new URLSearchParams({ [key]: value }).toString()}`;
}
function errorPath(error: { code?: string; message: string }) {
  ...
  if (error.code === "42501" || error.message.includes("permission_denied")) return destination("error", "access_denied");
...
export async function reserveAssetAction(formData: FormData) {
  ...
  if (error) redirect(errorPath(error)); refresh(); redirect(destination("notice", "reserved"));
}
...
export async function releaseAssetReservationAction(formData: FormData) {
  ...
  if (error) redirect(errorPath(error)); refresh(); redirect(destination("notice", "released"));
}
...
export async function transferAssetAction(formData: FormData) {
  ...
  if (!custodian) redirect(destination("error", "invalid_input"));
  ...
  if (error) redirect(errorPath(error)); refresh(); redirect(destination("notice", "custody"));
}
...
export async function inspectAssetAction(formData: FormData) {
  ...
  if (error) redirect(errorPath(error)); refresh(); redirect(destination("notice", "inspected"));
}
...
export async function changeAssetStatusAction(formData: FormData) {
  ...
  if (error) redirect(errorPath(error)); refresh(); redirect(destination("notice", "status"));
}
