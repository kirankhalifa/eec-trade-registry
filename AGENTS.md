...
The approved intermediated ordering model is recorded in `docs/adr/0017-intermediated-and-staff-assisted-ordering.md`. Public customers normally approach an authorized and appropriately licensed business; that business remains the EEC ordering party. A future explicitly authorized staff command may enter the requisition on behalf of the verified business without impersonating it. Until that command exists, use dealer-representative submission followed by staff review and do not invent a direct-row or shared-credential workaround.
