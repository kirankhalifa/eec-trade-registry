...
select is((select count(*)::integer from public.staff_role_permissions as rp join public.staff_roles as role on role.id = rp.staff_role_id where role.code = 'platform_administrator'), 4, 'platform administrator has four scoped permissions');
