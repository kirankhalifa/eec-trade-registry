...
select ok((select count(*) from public.audit_events where table_name = 'stock_transfers' and record_id = (select id from public.stock_transfers where source_request_id = 'f5000000-0000-0000-0000-000000000003')) >= 5, 'every transfer state change is audited');
